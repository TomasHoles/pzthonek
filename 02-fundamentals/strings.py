...
def datum():
    user_input = input("Zadej datum: ")
    result = user_input.replace("." or ".-","-")
    print("",result)
...
def souslovi():
    user_input_souslovi = input("Zadej souslovi: ")
    python = user_input_souslovi.lower().replace(" ","_")
    words = user_input_souslovi.lower().split()
    camel = words[0] + ''.join(word.capitalize() for word in words[1:])
    
    print("Python: ", python)
    print("Camel syntax: ",camel)
...
def generace_hesel(pocet_osob):
    for i in range(pocet_osob):
        ...
        i = velka_pismena + mala_pismena + specialni_znak + cislice
        print(i)
