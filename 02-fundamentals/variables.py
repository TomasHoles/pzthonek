...
print("Students Count: ",type(students_count), id(students_count))
print("Rating: ",type(rating),id(rating))
print("Is published: ",type(is_published),id(is_published))
...
decimal_value = myself_binary
print(f"Binární číslo {bin(myself_binary)} v desítkové soustavě je {decimal_value}")

new_value = myself_binary >> 2
print(f"Po posunu o 2 bity vpravo: {new_value}")

hex_number = 0x1A
result = myself_binary & hex_number
print(f"Binární součin čísla {bin(myself_binary)} a {bin(hex_number)} je {bin(result)}")
